// ────────────────────────────────────────────────────────────────────────────
//  /api/shaders — GET (list) + POST (upload)
// ────────────────────────────────────────────────────────────────────────────
import { NextRequest, NextResponse } from "next/server";
import { supabaseAdmin } from "@/lib/supabase";
import { computeHashes } from "@/lib/hash";
import { uploadShader } from "@/lib/storage";
...
// ── POST /api/shaders — multipart/form-data upload ────────────────────────
  ...
  try {
    ...
    if (customUrl) {
      ...
      if (!customFilename.endsWith(".zip")) {
        return NextResponse.json(
          { error: "El nombre de archivo debe terminar en .zip" },
          { status: 400 }
        );
      }
      ...
      if (!file.name.endsWith(".zip")) {
        return NextResponse.json(
          { error: "Only .zip files are allowed for shaders" },
          { status: 400 }
        );
      }
      ...
      // Upload to storage
      const { publicUrl } = await uploadShader(file.name, bytes);
      finalUrl = publicUrl;
    }

    // Upsert into database (by filename)
    const { data, error } = await supabaseAdmin
      .from("shaders")
      .upsert(
        {
    ...
    console.error("[shaders/POST]", err);
    return NextResponse.json(
